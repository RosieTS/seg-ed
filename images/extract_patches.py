...
TO DO: WSI and mask directories, and file names are currently hard-coded. Make these a command line option.
...
def get_patch_origin_coords(image, patch_size, stride):
    # Need to make sure taking the same dimensions from image and mask.
    # This function assumes we're taking patches from level_dimensions[2]
    ...
    x_coords = [x * stride[0] * 4 for x in range(0,(width // (stride[0]*4) - 1))]
    y_coords = [y * stride[1] * 4 for y in range(0,(height // (stride[1]*4) - 1))]
...
def extract_patch(image, x, y, patch_size = (224, 224)):

    extracted_patch = image.read_region((x,y), 2, patch_size, ).convert('RGB')
      
    return extracted_patch

def extract_mask(mask, x, y, patch_size = (224, 224)):
   
    patch = mask.read_region((x,y), 2, patch_size, ).convert('L')
...
#    print(mask_file_names)
    ...
    for image_file_name, mask_file_name in file_names:
        ...
        x_coords, y_coords = get_patch_origin_coords(image, patch_size, stride)
        #print(x_coords)
        #print(y_coords)
        ...
        for x in x_coords:
            for y in y_coords:
                ...
                extracted_patch = extract_patch(image, x, y, patch_size = patch_size)
                ...
                if background_fraction <= tol:
                    ...
                    extracted_mask = extract_mask(mask, x, y, patch_size = patch_size)
